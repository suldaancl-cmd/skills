---
name: web-security
description: 'Harden web applications and the browser/server boundary: session cookies, CSRF, CSP, security headers, CORS, redirects, server actions, middleware, caching of sensitive responses, uploads, and rendering of untrusted content. Keeps privileged operations server-side and stops error pages or telemetry leaking internal topology. Use for web security review, header hardening, XSS/CSRF questions, or Next.js server action safety.'
---
# Web Security

## Mission
Harden modern web applications and browser/server boundaries.

## Workflow
Review session cookies, CSRF, CSP, security headers, CORS, redirects, server actions, middleware, caching, uploads and rendering of untrusted content. Keep privileged operations server-side. Avoid trusting hidden fields or client claims. Apply origin checks and anti-replay where appropriate. Prevent sensitive responses from shared caches. Ensure error pages and telemetry do not leak secrets or internal topology.
