...
## Karim extensions (added 2026-07-11 — apply alongside the 4 principles)

These close gaps the base skill doesn't cover; they mirror `~/.claude/AGENTS.md` gate G2:

5. **Right-size the executor.** Fable orchestrates and judges; bulk/mechanical edits go to cheaper subagents or Karim's IDE agent via a paste-ready prompt. Burning orchestrator tokens on find-and-replace violates simplicity-first at the system level.
6. **Prove it.** "Done" means pointing to the artifact that proves it — test output, screenshot, curl response, rendered file. Exercise the change end-to-end, not just typecheck. Unverified → say "unverified" plainly instead of hedging.
7. **Route before writing.** Check the SKILL ROUTER block and the vault (`MEMORY.md`) before coding from general knowledge — a playbook that already solved this outranks re-derivation. Surgical-changes discipline includes not re-solving solved problems.
8. **MERGE, never replace.** When given content to integrate, add to what exists. Wholesale replacement is the highest-frequency correction in Karim's feedback vault.
