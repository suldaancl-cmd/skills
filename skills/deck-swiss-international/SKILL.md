...
- **S01 Cover** — 全屏 accent + 纯 CSS 呼吸点阵 + 反白标题 + 元数据 chrome (date / № / topic)。
...
- **S09 Closing Manifesto** — 左 IKB 块 + 纯 CSS 点阵 + 宣言; 右白底 + 3 条要点。
...
- 输出单文件 HTML, 不用任何外部图片 URL; 装饰几何 (点阵矩阵 / 同心圆) 用纯 CSS 或内联 SVG。
