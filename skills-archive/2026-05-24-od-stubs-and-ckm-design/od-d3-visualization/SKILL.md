---
name: od-d3-visualization
description: |
  Teaches the agent to produce D3 charts and interactive data visualizations. Useful for editorial dashboards, reports, and explanatory graphics.
triggers:
  - "d3"
  - "d3.js"
  - "interactive chart"
  - "data visualization"
  - "editorial chart"
od:
  mode: prototype
  category: diagrams
  upstream: "https://github.com/chrisvoncsefalvay/d3-claude-skill"
---

> Codex import: this local copy is namespaced as `od-d3-visualization` to avoid collisions with existing skills. Upstream source: `nexu-io/open-design/skills/d3-visualization`.

# d3-visualization

> Curated from @chrisvoncsefalvay.

## What it does

Teaches the agent to produce D3 charts and interactive data visualizations. Useful for editorial dashboards, reports, and explanatory graphics.

## Source

- Upstream: https://github.com/chrisvoncsefalvay/d3-claude-skill
- Category: `diagrams`

## How to use

This catalogue entry advertises the skill in Open Design so the agent
discovers it during planning. To run the full upstream workflow with
its original assets, scripts, and references, install the upstream
bundle into your active agent's skills directory:

```bash
# Inspect the upstream README for exact paths
open https://github.com/chrisvoncsefalvay/d3-claude-skill
```

Then ask the agent to invoke this skill by name (`d3-visualization`) or with
one of the trigger phrases listed in this skill's frontmatter.

