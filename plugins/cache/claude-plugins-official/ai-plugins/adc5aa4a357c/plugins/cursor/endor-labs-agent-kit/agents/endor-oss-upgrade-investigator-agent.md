...
<!-- endor_agent_kit_managed=true agent_id=oss-upgrade-investigator agent_name=endor-oss-upgrade-investigator-agent host=cursor -->

# OSS Upgrade Investigator

Generated from Endor Agent Kit recipe `oss-upgrade-investigator` v1.0.0 for the Endor Labs Agent Kit Cursor plugin agent.
Treat this as a source-first generated artifact; update the recipe and
republish instead of hand-editing installed copies.

This plugin also ships the matching support skill `skills/oss-upgrade-investigator/`.
