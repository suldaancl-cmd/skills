---
name: routines
description: Create, fire, and manage Codex cloud routines (scheduled/API/GitHub-triggered remote agents). Use when Nick asks to set up a recurring remote agent, a webhook-triggered workflow, automate a daily/weekly task in the cloud, schedule a cron job for Codex, or anything involving Codex.ai/code/scheduled. Triggers on "routine", "schedule a remote agent", "webhook to Codex", "daily/weekly Codex job", "cron routine", or /routines.
allowed-tools: Read, Write, Edit, Grep, Glob, Bash, ToolSearch, WebFetch, RemoteTrigger
---

# Routines — Cloud Scheduled Agents

Routines are **saved Codex configurations that run on Anthropic-managed cloud infrastructure** — a prompt + repo(s) + connectors, fired on a schedule, HTTP POST, or GitHub event. They keep working when Nick's laptop is closed.
...
"Missing INSTANTLY_API_KEY — add at Codex.ai/code/environments" and exit.
```
...
**Cannot delete via API**. Direct Nick to https://Codex.ai/code/scheduled to delete.
        ...
        "ccr": {
            ...
            "events": [{
                "data": {
                    ...
                    "message": {
                        "role": "user",
                        "content": "Run WebSearch queries for 'Nick Saraev' with after:{today-2} date filter across reddit, x, HN, linkedin, youtube, podcasts. Filter hard for newsworthy signal (notable mentions, press, complaints, big student wins). Commit findings to Codex/nick-scan-YYYY-MM-DD branch if anything interesting, else print 'No notable mentions.' and exit."
                    }
                }
            }]
        }
