...
```bash
~/.Codex/skills/gstack/bin/gstack-question-log '{"skill":"design-shotgun","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
[ -z "$D" ] && D="$HOME/.Codex/skills/gstack/design/dist/design"
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
the legacy approved.json aggregate — `~/.Codex/skills/gstack/bin/gstack-taste-update`
...
variant, call `~/.Codex/skills/gstack/bin/gstack-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `~/.Codex/skills/gstack/bin/gstack-taste-update rejected <variant-path>`.
