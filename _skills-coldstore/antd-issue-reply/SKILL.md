...
> Codex import: local Ant Design skill from `C:\tmp\ant-design\.agents\skills\issue-reply`. Namespaced when needed to avoid collisions.
...
**处理功能请求时，先排查现有能力再讨论新增：**

1. **先查现有 API** — 用户提需求时常因"找不到"而误以为"没有"。收到 feature request 时，第一反应应是"我们是不是已经有这个了"，而非"怎么实现它"。搜一下相关组件的 props、文档、已有 issue，确认未被现有功能覆盖。
2. **关注命名可发现性** — 如果功能已存在但用户找不到，问题的本质不是缺功能而是 API 可发现性差。此时应该：向用户指出已有 API 并附代码示例；同时反思文档是否需要补充搜索关键词或交叉引用（如用户搜 `mapping` 但 API 叫 `fieldNames`，文档中应关联这两个词）。
3. **检查跨组件一致性** — 用户会从一个组件的 API 类推另一个。同一概念在不同组件用不同命名（如 Table 的 `rowKey` vs Select 的 `fieldNames`），会制造认知负担。如果发现用户因跨组件命名不一致而困惑，记录下来作为后续 API 一致性优化的参考。
4. **用提问引导而非否定** — 确认已有功能后，不要直接说"我们已经有这个了"然后关 issue。用提问式引导让用户自己确认："看描述这个需求好像可以用 `fieldNames` 实现，是我理解的不对吗？"——既帮用户解决问题，又保护了用户面子。用户确认后再关闭。
