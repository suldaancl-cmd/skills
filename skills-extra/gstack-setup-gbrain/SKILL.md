...
Codex-targeted; register `gbrain serve` in your agent's MCP config
...
```bash
~/.Codex/skills/gstack/bin/gstack-brain-init
~/.Codex/skills/gstack/bin/gstack-config set gbrain_sync_mode artifacts-only
# or "full" if user picked yes-full
```

Then wire the brain repo into gbrain so its content is searchable from any
gbrain client (this Codex session, future Macs, optional cloud agents).
